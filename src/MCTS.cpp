...
MCTS::MCTS(Model *net, float c_puct) :  _c_puct(c_puct) {
    _net = net;
}
...
LeafResult MCTS::find_leaf(const State &state, Player player) {
    ...
    while (!is_leaf(current_state)) {
        ...
        Vector7f counts = _visit_count.at(current_state).cast<float>();
        float total_sqrt = std::sqrt(counts.sum());
        Vector7f probs = _probs.at(current_state);
        Vector7f values_avg = _value_avg.at(current_state);
        ...
        std::vector<int64_t> invalid_acts = board.invalid_actions();
        for (int64_t act : invalid_acts)
            score[act] = -inf_float;
        int64_t action;
        score.maxCoeff(&action);
        actions.push_back(action);
        MoveResult move = board.move(action, current_player);
        ...
        // Check for draw
        if (value == inf_float && board.possible_moves_count() == 0)
            value = 0.0;
    }
...
void MCTS::search_batch(int64_t count, int64_t batch_size, const State &state, Player player) {
    for (int64_t i = 0; i < count; i++) {
        search_minibatch(batch_size, state, player);
    }
}

void MCTS::search_minibatch(int64_t count, const State &state, Player player) {
    ...
    for (int i = 0; i < count; i++) {
        LeafResult result = find_leaf(state, player);
    ...
    if (!expand_queue.empty()) {
        ...
        torch::Tensor value_tensor = pair.tensor2;
        ...
        for (uint64_t i = 0; i < expand_queue.size(); i++) {
            ...
            _probs[sat.state] = toEigen<Vector7f>(prob_tensor[i]);
            backup_queue.emplace_back(value_tensor[i][0].item<float>(), sat.states, sat.actions);
        }
    }
    ...
    for (const auto& queue : backup_queue) {
        ...
        for (int64_t i = queue.states.size() - 1; i >= 0; i--) {
            ...
            _visit_count.at(state)[action] += 1;
            _value.at(state)[action] += current_value;
            _value_avg.at(state)[action] = _value[state][action] / _visit_count[state][action];
...
VP MCTS::get_policy_value(const State &state, int64_t tau) {
    auto counts = _visit_count.at(state);
    ...
    if (tau == 0) {
        ...
        probs = Eigen::pow(counts.cast<float>(), 1.0f / tau);
        float total = probs.sum();
        probs = probs / total;
    }
    auto values = _value_avg.at(state);
    return {values, probs};
}
